f_mat_cust <- function(x, connection, ilevel, iYYYY, ifreq) {
  phantom <- x[1]
  iorg_level <- x[2]
  customer <- x[3]
  status_message <- 0
...
fcstMat_regi <- function( connection , Phantom, org_level, customer, query,  intermittent, DateMask, yrfreq, status) {
