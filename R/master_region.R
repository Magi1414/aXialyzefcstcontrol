f_mast_reg <- function(x, connection, ilevel, iYYYY, ifreq) {
  phantom <- x[1]
  iorg_level <- x[2]
  statusmessage <- 0
  statusmessage$status <- 'Initialized'
  statusmessage$message <- 'Initialized'
  iquery <- "select requested_deliv_date,liters from get_orderqty_per_date_inclmaterialmaster_cluster($1,$2,$3)"
  fcaccuracy <- extTryCatch(fcstMaster_reg(connection , phantom, iorg_level, iquery, FALSE, iYYYY, ifreq , statusmessage))
  print(x[1])
  write_fcobject_todb(connection, fcaccuracy, ilevel, phantom, iorg_level, iYYYY)
  print(x[1])
}

fcstMaster_reg <- function( connection , Phantom, org_level, query,  intermittent, DateMask, yrfreq, status) {
