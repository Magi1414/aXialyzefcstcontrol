f_mast_cust <- function(x, connection, ilevel, iYYYY, ifreq) {
  phantom <- x[1]
  iorg_level <- x[2]
  customer <- x[3]
  statusmessage <- 0
  ...
  iquery <- "select requested_deliv_date,liters from get_orderqty_per_date_inclmaterialmaster_cluster_payer($1,$2,$3,$4)"
  fcaccuracy <- extTryCatch(fcstMaster_cu(connection , phantom, iorg_level,customer, iquery, FALSE, iYYYY, ifreq , statusmessage))
  print(x[1])
  write_fcobject_todb(connection, fcaccuracy, ilevel, phantom, paste(paste(iorg_level," - "),customer), iYYYY)
  print(x[1])
}

fcstMaster_cu <- function( connection , Phantom, org_level,customer, query,  intermittent, DateMask, yrfreq, status) {
