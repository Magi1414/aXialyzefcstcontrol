...
df <- dbGetQuery(con, "SELECT distinct sales_org || ' - ' || business_grp  geography, 'price' material,
                 ...
                 left join revenue_vol_bw_price v on v.bw_date= to_char(ss,'YYYYMM')
                 where
                 ...
                 and f.material = 'price'
                 and geography ilike '%- ' || v.business_grp
                 and f.fcperiod = to_char(ss + '1 month' ::interval,'MM.YYYY'))

                 order by geography, requested_deliv_date_to
...
run_mat_cust_mm <- function(df, no_cores) {
  foreach(i=1:no_cores) %dopar%
  {
    ...
    source('~/R/aXialyzefcstcontrol/R/bw revenue_volume/revenue_salesorg_price.r')
