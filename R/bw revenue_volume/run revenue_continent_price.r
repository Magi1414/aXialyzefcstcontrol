...
df <- dbGetQuery(con, "SELECT distinct continent  geography, business_grp material,
                 ...
                 left join revenue_vol_bw_price_continent v on v.bw_date= to_char(ss,'YYYYMM')
                 where

                 not exists  (select material || geography from fcst_accuracy f where fcrun = (select fcrun_revenue from current_run)
                 and f.material = v.business_grp
                 and geography = continent
...
run_mat_cust_mm <- function(df, no_cores) {
  foreach(i=1:no_cores) %dopar%
  {
    ...
    source('~/R/aXialyzefcstcontrol/R/bw revenue_volume/revenue_continent_price.r')
