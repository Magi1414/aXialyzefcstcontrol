...
df <- dbGetQuery(con, "SELECT distinct sales_org || ' - ' || business_grp  geography, material,
		0 sma_only, 0 revenue, 0 cum_sum_percentage,
                 to_char(ss,'YYYY-MM') requested_deliv_date_to, to_char(ss + '1 month' ::interval,'MM.YYYY') fcperiod , business_grp
                 ...
                 left join revenue_vol_mat_bw_price_m v on v.bw_date= to_char(ss,'YYYYMM')
                 where
...
run_mat_cust_mm <- function(df, no_cores) {
  foreach(i=1:no_cores) %dopar%
  {
    ...
    source('~/R/aXialyzefcstcontrol/R/bw revenue_volume/revenue_material_salesorg.r')
    ...
    df2 <-  dbGetQuery(con, "SeLECT fcrun FROM current_run" )
