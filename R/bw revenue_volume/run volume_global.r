...
df <- dbGetQuery(con, "SELECT distinct business_grp , sales_org material, 0 sma_only, 0 revenue, 0 cum_sum_percentage, to_date() , paladium_y_n
                 from generate_series()
                    left join revenue_vol_bw_vol_glb on
                      left join (select paladium_y_n from paladium_scenarios) on 1 = 1
                 " )
...
run_mat_cust_mm <- function(df, no_cores) {
  foreach(i=1:no_cores) %dopar%
  {
    ...
    source('~/R/aXialyzefcstcontrol/R/bw revenue_volume/volume_global.R')
    ...
    df2 <-  dbGetQuery(con, "SELECT fcrun, to_char(requested_deliv_date_to, 'yyyy-mm-dd') as requested_deliv_date_to , fcperiod
                       FROM public.parameter_sets ps where ps.fcrun = (SeLECT fcrun_volume_bw_glb FROM current_run)" )
