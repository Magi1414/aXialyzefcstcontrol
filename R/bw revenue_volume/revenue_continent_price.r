f_mat_regi <- function(x, connection, ilevel, iYYYY, ifreq, sendfcserie, fcrun) {
  ...
  join revenue_vol_bw_price_continent b on b.bw_date = trim(to_char(a,'YYYYMM'))
  where continent = $2 and business_grp = $3
  order by 1"
...
fcstMat_region <- function( connection , Phantom, org_level, query,  intermittent, sma_only, DateMask, yrfreq, status, todate,business_grp) {
  ...
  df_postgres <- RPostgreSQL::dbGetQuery(connection,  query, c( todate,org_level,Phantom))
