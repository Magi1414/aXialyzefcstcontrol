f_mast_gl <- function(x, connection,ilevel,iorg_level,  iYYYY, ifreq) {
  phantom <- x[1]
  statusmessage <- 0
  statusmessage$status <- 'Initialized'
  statusmessage$message <- 'Initialized'
  iquery <- "select requested_deliv_date,liters from get_orderqty_per_date_inclmaterialmaster($1,$2)"
  fcaccuracy <- extTryCatch(fcstMaster_gl(connection , phantom,  iquery, FALSE, iYYYY, ifreq , statusmessage))
  print(x[1])
  write_fcobject_todb(connection, fcaccuracy, ilevel, phantom, iorg_level, iYYYY)
  print(x[1])
}

fcstMaster_gl <- function( connection , Phantom,  query,  intermittent, DateMask, yrfreq, status) {
