...
df <- dbGetQuery(con, "SELECT distinct cluster, 'revenue' material, 0 sma_only, 0 revenue, 0 cum_sum_percentage
                 from revenue_p_month_filtred" )
...
run_mat_cust_mm <- function(df, no_cores) {
  foreach(i=1:no_cores) %dopar%
  {
    ...
    source('~/R/aXialyzefcstcontrol/R/revenueInvoice.R')
    ...
    df2 <-  dbGetQuery(con, "SELECT fcrun, to_char(requested_deliv_date_to, 'yyyy-mm-dd') as requested_deliv_date_to , fcperiod
                       FROM public.parameter_sets ps where ps.fcrun = (SeLECT fcrun_revenue_invoice FROM current_run)" )
