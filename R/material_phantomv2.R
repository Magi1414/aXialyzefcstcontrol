f_mat_phant <- function(x, connection, ilevel, iYYYY, ifreq, fcperiod, sendfcserie, fcrun, todate) {
  phantom <- x[1]
  iorg_level <- x[2]
  region <- x[3]
  sma_only = FALSE
  ...
  iquery <- "select requested_deliv_date,liters from get_orderqtyvalue_per_date_inclmaterial_cluster_region_phantom($1,$2,$3,$4,$5)"
