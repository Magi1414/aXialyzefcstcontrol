...
df <- dbGetQuery(con, "SELECT material, cluster,  kilo,  time_serie_category
                 FROM public.time_serie_categories_cluster_month where time_serie_category = 'Continuous'" )
# Calculate the number of cores
no_cores <- detectCores() - 1

run_mat_regi_mm <- function(df, no_cores) {
    foreach(i=1:no_cores) %dopar%
    {
      ...
      source('~/aXialyzefcstcontrol/R/material_region.R')
      drv <- dbDriver("PostgreSQL")
      con <- dbConnect(drv, dbname = "Atotech",
                       host = "axialyzeproduction.c5drkcatbgmm.eu-central-1.rds.amazonaws.com", port = 8080,
                       user = "aXialyze", password = "aXialyze0000")
     ...
     apply(dfall, 1, f_mat_regi, connection = con, ilevel = level,  iYYYY = "YYYY-MM"  ,ifreq = 12)
    }}
# Initiate cluster
cl <- makeCluster(no_cores)
doParallel::registerDoParallel(cl)
tryCatch(run_mat_regi_mm(df, no_cores), error = function(e) print(e))
