...
df <- dbGetQuery(con, "SELECT material, cluster,  customer_code, totalvolume,  ts_categorie, case when sma_only then 1 else 0 end
                 ...
                 and not (division = '01' AND cluster = 'China') and totalvolume < 100
...
run_mat_cust_mm <- function(df, no_cores) {
    foreach(i=1:no_cores) %dopar%
    {
      ...
      con <- dbConnect(drv, dbname = "Atotech",
                       host = "axialyzeproduction.c5drkcatbgmm.eu-central-1.rds.amazonaws.com", port = 8080,
                       user = "aXialyze", password = "aXialyze0000")
      df2 <-  dbGetQuery(con, "SELECT fcrun, requested_deliv_date_to, fcperiod FROM public.parameter_sets ps where ps.fcrun = (SeLECT fcrun FROM current_run)" )
      ...
      apply(dfall, 1, f_mat_cust, connection = con, ilevel = level,  iYYYY = "YYYY-MM"  ,ifreq = 12, fcperiod, TRUE, fcrun, todate)
    }}
# Initiate cluster
